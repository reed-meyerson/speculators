...
## [Pretraining a Draft Model](pretraining.md)

Train a DFlash-family draft on raw text with no verifier forward pass -- no vLLM server, no hidden-state extraction -- then warm-start an ordinary distillation run from the result.
